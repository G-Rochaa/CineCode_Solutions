...
public class ControllerFilmesEmCartaz {
	...
	List<Filme> filmes = daoFilme.obterTodosEmCartaz();
	...
	public void initialize() {
		HBoxTituloLabel.setText("Filmes em Cartaz");
		...
		for (Filme filme : filmes) {
			...
			buttonUpdate.setOnAction(e -> {
				try {
					...
					FXMLLoader loader = new FXMLLoader(getClass().getResource("/View/EditarFilmesEmCartaz.fxml"));
					Parent subPage = loader.load();
					
					ControllerEditarFilmeEmCartaz controller = loader.getController();
					controller.initialize(filme);
					controller.setVboxFilmesCartaz(regiaoCentral);

					String css = this.getClass().getResource("/View/EditarFilmesEmCartaz.css").toExternalForm();
